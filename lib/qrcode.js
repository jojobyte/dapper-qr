 ...
 * @author davidshimjs, papnkukn, adrien-powder, leMaik, jp-ed, jojobyte
...
function QRCode(options) {
  ...
  function _getTypeNumber(content, ecl) {
    var length = toUTF8(content).length;
...
QRCode.prototype.svg = function (opt) {
  ...
  var styles = `fill="${options.color}" stroke="${options.strokeColor}" shape-rendering="${options.shapeRendering}" stroke-linecap="round" stroke-linejoin="round" stroke-width="${options.strokeWidth}"`;

  if (!options.useAttrs) {
    styles = `style="fill:${options.color};stroke:${options.strokeColor};shape-rendering:${options.shapeRendering};stroke-linecap:round;stroke-linejoin:round;stroke-width:${options.strokeWidth};"`;
  }
  ...
  const holeSize = options.hole >= 0 ? options.hole : 0;
  const holeMin = (length / 2) - (holeSize/2);
  const holeMax = holeMin + holeSize;
      ...
      if (module) {
        ...
        if (join) {
          ...
          modrect +=
            ...
            // 4 +
            xsize/2.5 +
            // (xsize/2 + ysize^2/(8*xsize)) +
            '" ' +
            styles +
            '/>' +
            EOL;
          } else if (!options.dotStyle || options.dotStyle === 'rect') {
